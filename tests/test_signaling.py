...
def test_two_peers_join_and_relay():
    with TestClient(app) as client:
        with client.websocket_connect("/live-call/ws/signal/r1") as a:
            assert a.receive_json() == {"type": "joined", "role": "caller"}
            with client.websocket_connect("/live-call/ws/signal/r1") as b:
...
def test_third_peer_rejected():
    with TestClient(app) as client:
        with client.websocket_connect("/live-call/ws/signal/r2") as a, \
             client.websocket_connect("/live-call/ws/signal/r2") as b:
            a.receive_json(); b.receive_json(); a.receive_json(); b.receive_json()
            with client.websocket_connect("/live-call/ws/signal/r2") as c:
